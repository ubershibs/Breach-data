...
const studentsToProcess = []; // Example student numbers to process
...
fs.createReadStream(path.join(__dirname, 'src', 'data', 'students.csv'))
  ...
  .on('data', (row) => {
    ...
    if (row.Student_Number) {
      studentsToProcessRows.push(row.Student_Number);
    }
  })
  .on('end', async () => {
...
*/
async function generatePdfs(studentsToProcess) {
  ...
  for (const studentNumber of studentsToProcess) {
    const url = `https://localhost:3443/studentnumber?studentnumber=${studentNumber}`;
...
generatePdfs(studentsToProcess);